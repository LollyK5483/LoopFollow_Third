...
class AlarmViewController: FormViewController {
    ...
    override func viewDidLoad() {
        ...
        <<< SegmentedRow<String>("otherAlerts4"){ row in
            row.title = ""
            row.options = ["Rec. Bolus", "Temp Target Start", "Temp Target End"]
            if !IsNightscoutEnabled() {
                row.hidden = true
            }
        }.onChange { [weak self] row in
    ...
    func buildTempTargetStart() {
        form
        +++ Section(header: "Temp Target Started Alert", footer: "Alert will trigger without repeat once when Temp Target is activated. There is no need to snooze this alert") { row in
            row.hidden = "$otherAlerts4 != 'Temp Target Start'"
        }
        <<< SwitchRow("alertTempTargetStart"){ row in
            row.title = "Active"
            row.value = UserDefaultsRepository.alertTempTargetStart.value
        }.onChange { [weak self] row in
            guard let value = row.value else { return }
            UserDefaultsRepository.alertTempTargetStart.value = value
        }

        <<< PickerInputRow<String>("alertTempTargetStartSound") { row in
            row.title = "Sound"
            row.options = soundFiles
            row.value = UserDefaultsRepository.alertTempTargetStartSound.value
            row.displayValueFor = { value in
                guard let value = value else { return nil }
                return "\(String(value.replacingOccurrences(of: "_", with: " ")))"
            }
            ...
            guard let value = row.value else { return }
            UserDefaultsRepository.alertTempTargetStartSound.value = value
            AlarmSound.setSoundFile(str: value)
            AlarmSound.stop()
            AlarmSound.playTest()
        }
        <<< PickerInputRow<String>("alertTempTargetStartPlaySound") { row in
            row.title = "Play Sound"
            row.options = alertPlaySoundOptions
            row.value = UserDefaultsRepository.alertTempTargetStartAudible.value
            row.displayValueFor = { value in
                guard let value = value else { return nil }
                return "\(String(value.replacingOccurrences(of: "_", with: " ")))"
            }
            ...
            guard let value = row.value else { return }
            UserDefaultsRepository.alertTempTargetStartAudible.value = value
            let alertVol = self!.timeBasedSettings(pickerValue: value)
            UserDefaultsRepository.alertTempTargetStartDayTimeAudible.value = alertVol.dayTime
            UserDefaultsRepository.alertTempTargetStartNightTimeAudible.value = alertVol.nightTime
        }
        <<< PickerInputRow<String>("alertTempTargetStartRepeat") { row in
            row.title = "Repeat Sound"
            row.options = alertRepeatOptions
            row.value = UserDefaultsRepository.alertTempTargetStartRepeat.value
            row.displayValueFor = { value in
                guard let value = value else { return nil }
                return "\(String(value.replacingOccurrences(of: "_", with: " ")))"
            }
            ...
            guard let value = row.value else { return }
            UserDefaultsRepository.alertTempTargetStartRepeat.value = value
            let alertTimes = self!.timeBasedSettings(pickerValue: value)
            UserDefaultsRepository.alertTempTargetStartDayTime.value = alertTimes.dayTime
            UserDefaultsRepository.alertTempTargetStartNightTime.value = alertTimes.nightTime

        }
        <<< PickerInputRow<String>("alertTempTargetStartAutoSnooze") { row in
            row.title = "Pre-Snooze"
            row.options = alertAutosnoozeOptions
            row.value = UserDefaultsRepository.alertTempTargetStartAutosnooze.value
            row.displayValueFor = { value in
                guard let value = value else { return nil }
                return "\(String(value.replacingOccurrences(of: "_", with: " ")))"
            }
            ...
            guard let value = row.value else { return }
            UserDefaultsRepository.alertTempTargetStartAutosnooze.value = value
            let alertTimes = self!.timeBasedSettings(pickerValue: value)
        ...
        <<< DateTimeInlineRow("alertTempTargetStartSnoozedTime") { row in
            row.title = "Snoozed Until"
            if (UserDefaultsRepository.alertTempTargetStartSnoozedTime.value != nil) {
                row.value = UserDefaultsRepository.alertTempTargetStartSnoozedTime.value
            }
            row.minuteInterval = 5
            row.noValueDisplayText = "Not Snoozed"
        }
        .onChange { [weak self] row in
            guard let value = row.value else { return }
            UserDefaultsRepository.alertTempTargetStartSnoozedTime.value = value
            UserDefaultsRepository.alertTempTargetStartIsSnoozed.value = true
            let otherRow = self?.form.rowBy(tag: "alertTempTargetStartIsSnoozed") as! SwitchRow
            otherRow.value = true
            otherRow.reload()
        }
        .onExpandInlineRow { [weak self] cell, row, inlineRow in
        ...
        <<< SwitchRow("alertTempTargetStartIsSnoozed"){ row in
            row.title = "Is Snoozed"
            row.value = UserDefaultsRepository.alertTempTargetStartIsSnoozed.value
            row.hidden = "$alertTempTargetStartSnoozedTime == nil"
        }.onChange { [weak self] row in
            guard let value = row.value else { return }
            UserDefaultsRepository.alertTempTargetStartIsSnoozed.value = value
            if !value {
                UserDefaultsRepository.alertTempTargetStartSnoozedTime.setNil(key: "alertTempTargetStartSnoozedTime")
                let otherRow = self?.form.rowBy(tag: "alertTempTargetStartSnoozedTime") as! DateTimeInlineRow
                otherRow.value = nil
                otherRow.reload()
            }
        }
    ...
    func buildTempTargetEnd() {
        form
        +++ Section(header: "Temp Target Ended Alert", footer: "Alert will trigger without repeat once when Temp Target is turned off. There is no need to snooze this alert") { row in
            row.hidden = "$otherAlerts4 != 'Temp Target End'"
        }
        <<< SwitchRow("alertTempTargetEnd"){ row in
            row.title = "Active"
            row.value = UserDefaultsRepository.alertTempTargetEnd.value
        }.onChange { [weak self] row in
            guard let value = row.value else { return }
            UserDefaultsRepository.alertTempTargetEnd.value = value
        }

        <<< PickerInputRow<String>("alertTempTargetEndSound") { row in
            row.title = "Sound"
            row.options = soundFiles
            row.value = UserDefaultsRepository.alertTempTargetEndSound.value
            row.displayValueFor = { value in
                guard let value = value else { return nil }
                return "\(String(value.replacingOccurrences(of: "_", with: " ")))"
            }
            ...
            guard let value = row.value else { return }
            UserDefaultsRepository.alertTempTargetEndSound.value = value
            AlarmSound.setSoundFile(str: value)
            AlarmSound.stop()
            AlarmSound.playTest()
        }
        <<< PickerInputRow<String>("alertTempTargetEndPlaySound") { row in
            row.title = "Play Sound"
            row.options = alertPlaySoundOptions
            row.value = UserDefaultsRepository.alertTempTargetEndAudible.value
            row.displayValueFor = { value in
                guard let value = value else { return nil }
                return "\(String(value.replacingOccurrences(of: "_", with: " ")))"
            }
            ...
            guard let value = row.value else { return }
            UserDefaultsRepository.alertTempTargetEndAudible.value = value
            let alertVol = self!.timeBasedSettings(pickerValue: value)
            UserDefaultsRepository.alertTempTargetEndDayTimeAudible.value = alertVol.dayTime
            UserDefaultsRepository.alertTempTargetEndNightTimeAudible.value = alertVol.nightTime
        }
        <<< PickerInputRow<String>("alertTempTargetEndRepeat") { row in
            row.title = "Repeat Sound"
            row.options = alertRepeatOptions
            row.value = UserDefaultsRepository.alertTempTargetEndRepeat.value
            row.displayValueFor = { value in
                guard let value = value else { return nil }
                return "\(String(value.replacingOccurrences(of: "_", with: " ")))"
            }
            ...
            guard let value = row.value else { return }
            UserDefaultsRepository.alertTempTargetEndRepeat.value = value
            let alertTimes = self!.timeBasedSettings(pickerValue: value)
        ...
        <<< PickerInputRow<String>("alertTempTargetEndAutoSnooze") { row in
            row.title = "Pre-Snooze"
            row.options = alertAutosnoozeOptions
            row.value = UserDefaultsRepository.alertTempTargetEndAutosnooze.value
            row.displayValueFor = { value in
                guard let value = value else { return nil }
                return "\(String(value.replacingOccurrences(of: "_", with: " ")))"
            }
            ...
            guard let value = row.value else { return }
            UserDefaultsRepository.alertTempTargetEndAutosnooze.value = value
            let alertTimes = self!.timeBasedSettings(pickerValue: value)
        ...
        <<< DateTimeInlineRow("alertTempTargetEndSnoozedTime") { row in
            row.title = "Snoozed Until"
            if (UserDefaultsRepository.alertTempTargetEndSnoozedTime.value != nil) {
                row.value = UserDefaultsRepository.alertTempTargetEndSnoozedTime.value
            }
            row.minuteInterval = 5
            row.noValueDisplayText = "Not Snoozed"
        }
        .onChange { [weak self] row in
            guard let value = row.value else { return }
            UserDefaultsRepository.alertTempTargetEndSnoozedTime.value = value
            UserDefaultsRepository.alertTempTargetEndIsSnoozed.value = true
            let otherRow = self?.form.rowBy(tag: "alertTempTargetEndIsSnoozed") as! SwitchRow
            otherRow.value = true
            otherRow.reload()
        }
        .onExpandInlineRow { [weak self] cell, row, inlineRow in
        ...
        <<< SwitchRow("alertTempTargetEndIsSnoozed"){ row in
            row.title = "Is Snoozed"
            row.value = UserDefaultsRepository.alertTempTargetEndIsSnoozed.value
            row.hidden = "$alertTempTargetEndSnoozedTime == nil"
        }.onChange { [weak self] row in
            guard let value = row.value else { return }
            UserDefaultsRepository.alertTempTargetEndIsSnoozed.value = value
            if !value {
                UserDefaultsRepository.alertTempTargetEndSnoozedTime.setNil(key: "alertTempTargetEndSnoozedTime")
                let otherRow = self?.form.rowBy(tag: "alertTempTargetEndSnoozedTime") as! DateTimeInlineRow
                otherRow.value = nil
                otherRow.reload()
            }
        }
